...
int main(){
	...
	if(continueOrNot == 1){
		...
		printf("%s",tickets[2][2]);
		printf("%s",tickets[3][3]);
		printf("%s",tickets[4][4]);
		...
		for(i = 0; i < MAX_TICKETS; i++){
			printf("%s %s %s %s %s",tickets[i][0],tickets[i][1],tickets[i][2],tickets[i][3],tickets[i][4]);
		}
	}	
